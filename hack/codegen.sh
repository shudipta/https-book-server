...
PACKAGE_NAME=github.com/soter/scanner
REPO_ROOT="$GOPATH/src/$PACKAGE_NAME"
DOCKER_REPO_ROOT="/go/src/$PACKAGE_NAME"
DOCKER_CODEGEN_PKG="/go/src/k8s.io/code-generator"
apiGroups=(scanner/v1alpha1)
...
docker run --rm -ti -u $(id -u):$(id -g) \
  ...
  github.com/soter/scanner/client \
  github.com/soter/scanner/apis \
  github.com/soter/scanner/apis \
  scanner:v1alpha1 \
...
docker run --rm -ti -u $(id -u):$(id -g) \
  -v "$REPO_ROOT":"$DOCKER_REPO_ROOT" \
  -w "$DOCKER_REPO_ROOT" \
  appscode/gengo:release-1.9 "$DOCKER_CODEGEN_PKG"/generate-groups.sh client \
  github.com/soter/scanner/client \
  github.com/soter/scanner/apis \
  scanner:v1alpha1 \
  --go-header-file "$DOCKER_REPO_ROOT/hack/gengo/boilerplate.go.txt"
